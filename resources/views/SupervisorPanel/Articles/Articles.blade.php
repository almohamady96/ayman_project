    ...
    <div class="container-fluid">
        ...
        <ol class="breadcrumb breadcrumb-bg-teal">
            ...
            @if(isset($category))
                ...
                <li class="active">مقالات {{$category->name}}</li>
            @else
                        ...
                        <ul class="header-dropdown m-r--5">
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu pull-left">
                                    <li>
                                        @if(isset($category))
                                            <a href="{{ URL::to('/SupervisorPanel/Categories/'.$category->id.'/Articles/CreateArticle') }}">إضافه
                                                مقال جديد</a>
                                        @else
                                            <a href="{{ URL::to('/SupervisorPanel/Articles/CreateArticle') }}">إضافه مقال
                                                جديد</a>
                                        @endif
                                    </li>
                    ...
                    <div class="body">
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                            ...
                            @foreach($articles as $article)
                                ...
                                {{\App\Category::where('id','=',$article->category_id)->exists() ? $article->category->name : 'تم حذف القسم'}}
                                    ...
                                    <td class="text-center">
                                        <?php
                                        if (isset($category)) {
                                            $baseLink = '/SupervisorPanel/Categories/' . $category->id;
                                        } else {
                                            $baseLink = '/SupervisorPanel';
                                        }
                                        ?>
                                        <a href="{{ URL::to($baseLink.'/Articles/'.$article->id.'/Edit') }}"
                                           class="btn bg-light-green waves-effect" data-toggle="tooltip"
                                           data-placement="top" title="تعديل {{$article->name}} "
                                           style="margin : 2px">
                                            <i class="material-icons">settings</i>
                                        </a>
                                        <button onclick="showConfirmMessage('{{ URL::to('/SupervisorPanel/Articles/'.$article->id.'/Delete') }}')"
                                                class="btn bg-red waves-effect"
                                                data-toggle="tooltip"
                                                data-placement="top" title=" حذف {{$article->name}}"
                                                style="margin : 2px">
